        ...
        <div class="modal-content modal-sm" style="margin:0 auto">
            ...
            <div class="modal-body">
                ...
                <select name="tahun" class="form-control" required>
                  <?php
                    $ahay = mysqli_query($kon, "SELECT DISTINCT YEAR(tglrusak) as tahun FROM inventorirusak ORDER BY tahun DESC");
                    while($baris = mysqli_fetch_array($ahay)) {
...
<div id="page-wrapper">
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header"><button class="btn btn-primary btn-lg"><a href="rusak_input.php" style="color: white; text-decoration: none">+</a></button> 
                <button type="button" class="btn btn-primary btn-lg" data-toggle="modal" data-target="#myModal"> <i class="fa fa-print"></i> </button> Inventori Rusak </h1>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-12">
                                    ...
                                    <?php $no=1; $query = mysqli_query($kon, "SELECT * FROM inventorirusak INNER JOIN inventori ON inventorirusak.idinventori = inventori.idinventori INNER JOIN user ON inventorirusak.id = user.id ORDER BY namainven ASC");
                                        while($data = mysqli_fetch_array($query)){ ?>
                                            <tr class="odd gradeX">
                                                    ...
                                                    <td><?= $data['jumlah'] ?></td>
                                                    <td>
                                                        <a href="rusak_edit.php?idinventorirusak=<?php echo $data['idinventorirusak']; ?>" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i></a>
                                                        <button class="btn btn-primary btn-sm" type="button" onclick="return confirm('Yakin ingin Menghapus?');"><a href="delete.php?idinventorirusak=<?php echo $data['idinventorirusak'] ?>" style="color: white;"><i class="fa fa-trash"></i></a></button>
                                                    </td>
                                                </tr>
                                        <?php } ?>
